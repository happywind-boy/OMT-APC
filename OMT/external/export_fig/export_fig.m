    ...
    try
        ...
        if options.svg
            ...
            try %if verLessThan('matlab', '8.4')
                ...
                % Try Matlab's built-in svg engine (from Batik Graphics3d for java)
                try
        ...
        if options.clipboard
            ...
            try
                ...
                try
                    ...
                    % Probably gray-scaled image (3d, without the 3rd [RGB] dimension)
        ...
        elseif ischar(varargin{a}) && ~isempty(varargin{a})
            if varargin{a}(1) == '-'
                switch lower(varargin{a}(2:end))
                    ...
                    case {'a1', 'a2', 'a3', 'a4'}
                        options.aa_factor = str3double(varargin{a}(3));
                    case 'append'
                        options.append = true;
                    case 'bookmark'
                    ...
                    otherwise
                        try
                            ...
                            elseif strcmpi(varargin{a}(1:2),'-c')
                                ...
                                val = str3double(regexp(varargin{a}, '(?<=-(m|M|r|R|q|Q|p|P))-?\d*.?\d+', 'match'));
                                if isempty(val) || isnan(val)
                                    % Issue #51: improved processing of input args (accept space between param name & value)
                                    val = str3double(varargin{a+1});
