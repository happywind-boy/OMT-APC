...
%        opt.ArrayToStruct[0|1]: when set to 0, saveubjson outputs 1D/3d
...
%                         is 1 for 1D or 3d arrays. If one wants to
...
if(~bitget(ismsgpack, 1))
    ...
    opt.IM_=char([hex3dec('cc') hex3dec('d0') hex3dec('d1') hex3dec('d2') hex3dec('d3')]);
    opt.FM_=char([hex3dec('ca') hex3dec('cb')]);
    opt.FTM_=char([hex3dec('c2') hex3dec('c3')]);
    opt.SM_=char([hex3dec('a1') hex3dec('db')]);
    opt.ZM_=char(hex3dec('c0'));
    opt.OM_={char(hex3dec('df')),''};
    opt.AM_={char(hex3dec('dd')),''};
end
...
dim=size(item);
if(ndims(squeeze(item))>2) % for 3D or higher dimensions, flatten to 3d for now
...
if(isunpack)
    ...
    if(~isoct)
        newname=regexprep(name,'(^x|_){1}0x([0-9a-fA-F]+)_','${native2unicode(hex3dec($2))}');
    else
        ...
        newname='';
        for i=1:length(pos)
            newname=[newname str0(pos0(i)+1:pos(i)-1) char(hex3dec(str0(pos(i)+3:pend(i)-1)))];
        end
