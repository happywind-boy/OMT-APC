...
%                         setting to 2 will return a cell array of 3d
%                         arrays; setting to 3 will return to a 3d cell
            ...
            if(jsonopt('FastArrayParser',1,varargin{:})>=1 && arraydepth>=jsonopt('FastArrayParser',1,varargin{:}))
                ...
                if(~isempty(endpos))
                    ...
                    if(isempty(find(arraystr=='"', 1)))
                        ...
                        % next handle 3d array, these are most common ones
                        if(maxlevel==2 && ~isempty(regexp(arraystr(2:end),'^\s*\[','once')))
                            rowstart=find(arraystr(2:end)=='[',1)+1;
                            if(rowstart)
                                [obj, nextidx]=parse3darray(inputstr,pos+rowstart,arraystr);
                        ...
                        % in the future can replace 1d and 3d cases
                        if(maxlevel>2 && ~isempty(regexp(arraystr(2:end),'^\s*\[\s*\[','once')))
                            astr=arraystr;
                            dims=nestbracket3dim(astr);
...
function newstr=unescapejsonstring(str)
    ...
    newstr=regexprep(newstr,'\\u([0-9A-Fa-f]{4})', '${char(base3dec($1,16))}');
end
...
%%-------------------------------------------------------------------------
function [obj, nextidx,nextdim]=parse3darray(inputstr,startpos,arraystr)
