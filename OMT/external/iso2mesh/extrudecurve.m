...
% 
% create a triangular surface mesh by swining a 3d spline along another 3d
% spline curve
%
% author: Qianqian Fang, <q.fang at neu.edu>
%
% input:
%      xy: a 3d spline path, along which the surface is extruded, defined
%          on the x-y plane
%      yz: a 3d spline which will move along the path to form a surface,
...
xyz(:,1)=0;
for i=1:length(xi)
    rot3d=[normaldir(i,1) -normaldir(i,2); normaldir(i,2) normaldir(i,1)];
    offset=[xi(i) yi(i) anchor(2)];
    newyz=[((rot3d*(xyz(:,1:2)-repmat(anchor(1:2),size(xyz,1),1))' + repmat(offset(:,1:2),length(zi),1)'))' , xyz(:,end)];
