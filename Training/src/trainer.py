...
# =============================== Multiple Raw crop training ===============================
def multi_train_rawcrop_step(ep, model, train_loader, optimizer, criterion, act, size, device):
    model.train()
    record = init_record(mode='multi_crop')
    train_bar = tqdm(train_loader, desc=f'Training {ep}')
    for data in train_bar:
        optimizer.zero_grad()
        image = data['rawimg'].to(device)
        label = data['rawlab'].to(device)
        ...
        batch_size = image.shape[0]
        record['BLoss'].update(loss, batch_size)
        record['wt_BAcc'].update(wt_acc.item(), batch_size)
        record['tc_BAcc'].update(tc_acc.item(), batch_size)
        record['et_BAcc'].update(et_acc.item(), batch_size)
...
def multi_val_rawcrop_step(ep, model, val_loader, optimizer, criterion, act, size, device):
    model.eval()
    record = init_record(mode='multi_crop')
    val_bar = tqdm(val_loader, desc=f'Validaion {ep}')
    with torch.no_grad():
        for index, data in enumerate(val_bar):
            ...
            pred = monai.inferers.sliding_window_inference(
                inputs=image, roi_size=(size, size, size), sw_batch_size=4, predictor=model)
            ...
            record['BLoss'].update(loss.item())
            record['wt_BAcc'].update(wt_acc.item())
            record['tc_BAcc'].update(tc_acc.item())
            record['et_BAcc'].update(et_acc.item())
...
# =============================== Raw crop training ===============================
def train_rawcrop_step(ep, model, train_loader, optimizer, criterion, act, size, device):
    model.train()
    record = init_record(mode='crop')
    train_bar = tqdm(train_loader, desc=f'Training {ep}')
    for data in train_bar:
        optimizer.zero_grad()
        image = data['rawimg'].to(device)
        ...
        batch_size = image.shape[0]
        record['BLoss'].update(loss.item(), batch_size)
        record['BAcc'].update(acc.item(), batch_size)
...
def val_rawcrop_step(ep, model, val_loader, optimizer, criterion, act, size, device):
    model.eval()
    record = init_record(mode='crop')
    val_bar = tqdm(val_loader, desc=f'Validaion {ep}')
    for data in val_bar:
